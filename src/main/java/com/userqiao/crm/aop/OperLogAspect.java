...
import com.userqiao.crm.utils.WebUtil;
...
public class OperLogAspect {
     ...
     */
    @Pointcut("@annotation(com.userqiao.crm.aop.OperLog)")
     ...
     */
    @Pointcut("execution(* com.userqiao.crm.controller..*.*(..))")
    ...
    public void saveOperLog(JoinPoint joinPoint, Object keys) {
        ...
        User user = UserUtils.getUser();//获取用户信息
        OperLogModel operlog = new OperLogModel();
        try {
            operlog.setOperId(IdUtil.simpleUUID()); // 主键ID
...
//            String params = JSON.toJSONString(rtnMap);
            ...
            operlog.setOperUserName(user.getName()); // 请求用户名称
            operlog.setOperIp(WebUtil.getIpAddr(request)); // 请求IP
    ...
    public void saveExceptionLog(JoinPoint joinPoint, Throwable e) {
        ...
        HttpServletRequest request = (HttpServletRequest) requestAttributes
                .resolveReference(RequestAttributes.REFERENCE_REQUEST);
        User user = UserUtils.getUser();//获取用户信息
        ExcLog excepLog = new ExcLog();
        try {
            // 从切面织入点处通过反射机制获取织入点处的方法
            MethodSignature signature = (MethodSignature) joinPoint.getSignature();
            // 获取切入点所在的方法
            Method method = signature.getMethod();
            excepLog.setExcId(RandomUtil.randomNumber());
            // 获取请求的类名
            String className = joinPoint.getTarget().getClass().getName();
            // 获取请求的方法名
            String methodName = method.getName();
            ...
            excepLog.setOperUserId(user.getId()); // 操作员ID
            excepLog.setOperUserName(user.getName()); // 操作员名称
            excepLog.setOperUrl(request.getRequestURI()); // 操作URI
            excepLog.setOperIp(WebUtil.getIpAddr(request)); // 操作员IP
